 ...
 */
function _showBusy(): void {
    const busy: any = document.getElementById('MM-Busy');
    if (!busy) {
        //cria style tag usada no busy
        createBusyStyle();

        let newBusy: any = document.createElement("div");
        newBusy.id = "MM-Busy";
        newBusy.className = "MM-Busy";

        //divs que serão as bolhas
        let bubble01 = document.createElement("div");
        let bubble02 = document.createElement("div");
        let bubble03 = document.createElement("div");
 ...
 */
function _hideBusy() {
    let busy = document.getElementById('MM-Busy');
 ...
 */
function _getIconClassByType(type: string): string {
    if (type == "info") { return "MM-IconInfo" }
    if (type == "success") { return "MM-IconSuccess" }
    if (type == "warning") { return "MM-IconAlert" }
    if (type == "error") { return "MM-IconError" }
 ...
 */
function _createFunctionToButton(func: any, modal: any) {
    return function () {
        if (func) { func(); }
        _closeModal(modal);
    }
 ...
 * @param {htmlObject} modal modal principal que está sendo criado
 */
function _closeModal(modal: any) {
    modal.children[0].style.animation = "hide 0.3s normal 0s linear";
